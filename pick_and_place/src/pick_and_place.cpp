...
int main(int argc, char** argv)
{
  ...
  ros::Publisher goal_pub = nh.advertise<geometry_msgs::PoseStamped>("/iiwa/command/CartesianPose", 10);
  ...
  while (true) {
      ...
      if (find_pick_client.call(srv))
      {
        ...
        ROS_ERROR_STREAM("Point Cloud processing took " << total_process << " s");
        //get target pose from ROS message and convert it to Eigen 3d vector.
      ...
      //Isometry is a type of transform which keeps distance between points
...
//        std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
        ...
        random_place.linear() = Eigen::Quaterniond(0, 0, 1, 0).matrix();
        random_place.translation() =
                Eigen::Vector3d(-0.2+0.04*generateRandom(), 0.4+0.03*generateRandom(), 0.77153 + box_size_z + gripper_offset);

        // Set the target pose to middle_right_shelf
        final_pose = random_place;
...
//          std::cin >> input;
    ...
    //        std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
            ...
            // Home the arm using provided ROS topic
    //        geometry_msgs::PoseStamped goal_msg;
    //        goal_msg.pose.position.x = 0.471;
    //        goal_msg.pose.position.y = 0.0;
    //        goal_msg.pose.position.z = 0.576;
    //        goal_msg.pose.orientation.y = 1.0;
    //        goal_pub.publish(goal_msg);

    //        auto arrived_cart = ros::topic::waitForMessage<std_msgs::Time>("/iiwa/state/DestinationReached");

            //ROS_WARN("Arm in home");
          }
        }
      }
      else
