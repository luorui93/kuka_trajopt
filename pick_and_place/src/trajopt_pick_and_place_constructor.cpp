...
TrajoptPickAndPlaceConstructor::TrajoptPickAndPlaceConstructor(tesseract::BasicEnvConstPtr env,
                                                               ...
                                                               std::string ee_link,  //end_effector_link
                                                     ...
                                                     int first_time_step)
{
  ...
  {
    /* Fill Code:
         . Create a new shared_ptr<CartPoseTermInfo>
    ...
    */
    /* ========  ENTER CODE HERE ======== */
    // ENTER CODE HERE:
//      std::shared_ptr<CartPoseTermInfo> pose_constraint(new CartPoseTermInfo);
//      auto pose_constraint = std::shared_ptr<CartPoseTermInfo> (new CartPoseTermInfo);
      //a better way to initialize shared_pointer
                                                                   ...
                                                                   int steps_per_phase)
{
  ...
  */
  /* ========  ENTER CODE HERE ======== */
  // ENTER CODE HERE:  the whole problem consists of two phases, so the total steps is steps_per_phase*2
  ...
  // ENTER CODE HERE:
  pci.basic_info.dt_lower_lim = 0.2;  //dt = 5
  // ENTER CODE HERE:
  pci.basic_info.dt_upper_lim = 0.5;  //dt = 2
  // ENTER CODE HERE:
  ...
  /* Fill Code:
       ...
       . Define the first time step   last time step - first time step defines how long will the cost/consraint apply
                                                                    ...
                                                                    int steps_per_phase)
{
  ...
  */
  ...
  // ENTER CODE HERE:
  pci.basic_info.dt_upper_lim = 0.5;
  // ENTER CODE HERE:
  pci.basic_info.use_time = false;
  ...
  */
  ...
  // ENTER CODE HERE:
  this->addLinearMotion(pci, approach_pose, final_pose, steps_per_phase, 2*steps_per_phase);
  TrajOptProbPtr result = ConstructProblem(pci);
  return result;
}

TrajOptProbPtr TrajoptPickAndPlaceConstructor::generateLinearMotion(Isometry3d& start_pose,
                                                                    Isometry3d& end_pose,
                                                                    int steps_per_phase)
{
  //---------------------------------------------------------
  ...
  */
  /* ========  ENTER CODE HERE ======== */
  // ENTER CODE HERE:  the whole problem consists of two phases, so the total steps is steps_per_phase*2
  pci.basic_info.n_steps = steps_per_phase;
  // ENTER CODE HERE:
  pci.basic_info.start_fixed = true;
  // ENTER CODE HERE:
  pci.basic_info.manip = manipulator_;
  ...
  */
  ...
  // ================= Path waypoints =======================
  this->addLinearMotion(pci, start_pose, end_pose, steps_per_phase, 0);
